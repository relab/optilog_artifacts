...
// Update is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (c *Configuration) Update(ctx context.Context, in *UpdateMsg, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "hotstuffpb.Hotstuff.Update",
	}
...
// ReconfigurationRequest is a quorum call invoked on all nodes in configuration c,
// with the same argument in, and returns a combined result.
func (c *Configuration) ReconfigurationRequest(ctx context.Context, in *ReconfigurationMsg, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "hotstuffpb.Hotstuff.ReconfigurationRequest",
	}
...
type Hotstuff interface {
	...
	Update(ctx gorums.ServerCtx, request *UpdateMsg)
	ReconfigurationRequest(ctx gorums.ServerCtx, request *ReconfigurationMsg)
...
func RegisterHotstuffServer(srv *gorums.Server, impl Hotstuff) {
	...
	srv.RegisterHandler("hotstuffpb.Hotstuff.Update", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*UpdateMsg)
		defer ctx.Release()
		impl.Update(ctx, req)
	})
	srv.RegisterHandler("hotstuffpb.Hotstuff.ReconfigurationRequest", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*ReconfigurationMsg)
		defer ctx.Release()
		impl.ReconfigurationRequest(ctx, req)
	})
