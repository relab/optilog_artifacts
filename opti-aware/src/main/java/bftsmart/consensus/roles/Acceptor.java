...
public final class Acceptor {
    ...
    public void proposeReceived(Epoch epoch, ConsensusMessage msg) {
        ...
        if (msg.getSender() == executionManager.getCurrentLeader() // Is the replica the leader?
            ...
            /** BEGIN AWARE */
            if (epoch.getConsensus().getId() > 1 && epoch.getConsensus().getId() % this.controller.getStaticConf().getCalculationInterval() == this.controller.getStaticConf().getCalculationDelay() + 1) {
    ...
    private void executePropose(Epoch epoch, byte[] value) {
        ...
        if (epoch.propValue == null) { // only accept one propose per epoch
            ...
            logger.trace("I have written value " + Arrays.toString(epoch.propValueHash) + " in consensus instance " + cid + " with timestamp " + epoch.getConsensus().getEts());
            /*****************************************/
            ...
            if (epoch.deserializedPropValue != null && !epoch.isWriteSent()) {
                ...
                if (controller.getStaticConf().isBFT()) {
                    ...
                    communication.send(this.controller.getCurrentViewOtherAcceptors(), factory.createAccept(cid, epoch.getTimestamp(), epoch.propValueHash));
    ...
    private void computeWrite(int cid, Epoch epoch, byte[] value) {
        ...
        logger.debug(" I have " + writeWeigths + " WRITEs weigths  and " + epoch.countWrite(value) + " WRITE messages for " + cid + "," + epoch.getTimestamp());

        // code for classic quorums
        // if (writeAccepted > controller.getQuorum() && Arrays.equals(value,
        // epoch.propValueHash)) {
    ...
    private void computeAccept(int cid, Epoch epoch, byte[] value) {
        ...
        logger.debug("I have " + acceptWeights + " ACCEPT weigths and " + epoch.countAccept(value) + " ACCEPT messages for " + cid + "," + epoch.getTimestamp());

        // normal code, for classic quorums
        // if (round.countAccept(value) > controller.getQuorum() &&
        ...
        if (acceptWeights - ((double) controller.getOverlayQuorum()) > THRESHOLD && !epoch.getConsensus().isDecided()) {
            ...
            // System.out.println("Decice " + cid + " weights: " + acceptWeights + " Qv: " +
            // controller.getOverlayQuorum());
